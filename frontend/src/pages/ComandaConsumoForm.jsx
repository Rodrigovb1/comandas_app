...
const ComandaConsumoForm = () => {
    ...
    const normalizeArray = (response) => response?.data || response?.items || response || []; // Regras de validação dos campos
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                setItens(normalizeArray(itensData));
                ...
                const produtosData = normalizeArray(response);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const produtoSelecionado = produtos.find(p => Number(p.id) === Number(data.produto_id));
            if (!produtoSelecionado) {
                showSnackbar('Selecione um produto válido', 'warning');
                return;
            }
        ...
        <PageLayout title={`Consumo - Comanda ${comanda?.comanda}`}>
            ...
            {comanda && (
                <Box sx={{ mb: 3, p: 2, backgroundColor: 'grey.100', borderRadius: 1 }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        Abertura: {formatLocalDateTime(comanda.data_hora)}
                    </Typography>
                </Box>
            )}
